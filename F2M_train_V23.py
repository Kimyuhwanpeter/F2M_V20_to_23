...
FLAGS = easydict.EasyDict({"img_size": 256, 
                           ...
                           "A_txt_path": "/content/train_BM.txt",
                           
                           "A_img_path": "/content/data/",
                           
                           "B_txt_path": "/content/train_WM.txt",
                           
                           "B_img_path": "/content/data/",
                           ...
                           "pre_checkpoint_path": "/content/drive/MyDrive/4th_paper/F_to_M/checkpoint_21_KL/28",
                           
                           "save_checkpoint": "/content/drive/MyDrive/4th_paper/F_to_M/checkpoint_21_KL",
                           
                           "sample_images": "/content/drive/MyDrive/4th_paper/F_to_M/sample_images_21_KL",
                           
                           "A_test_txt_path": "/content/test_BM.txt",
                           
                           "A_test_img_path": "/content/data/",
                           
                           "B_test_txt_path": "/content/test_WM.txt",
                           
                           "B_test_img_path": "/content/data/",
                           
                           "test_dir": "A2B",
                           
                           "fake_B_path": "/content/drive/MyDrive/4th_paper/F_to_M/fake_B",
                           
                           "fake_A_path": "/content/drive/MyDrive/4th_paper/F_to_M/fake_A"})
...
def cal_loss(A2B_model, B2A_model, DA_model, DB_model, DA_age_model, DB_age_model,
    ...
    with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:
        ...
        for i in range(FLAGS.batch_size):
            A_img = tf.reduce_mean(A_batch_images[i] * 0.5 + 0.5, -1)
            B_fa = tf.reduce_mean(fake_B[i] * 0.5 + 0.5, -1)
            B_img = tf.reduce_mean(B_batch_images[i] * 0.5 + 0.5, -1)
            A_fa = tf.reduce_mean(fake_A[i] * 0.5 + 0.5, -1)

            # energy_ft = (A_batch_images[i] - fake_B[i])**2
            # energy_ft2 = (B_batch_images[i] - fake_A[i])**2

            # realB_fakeB_loss = 0.5 * B_class_weights * tf.keras.losses.KLDivergence()(B_img, (A_fa+B_img)/2.) \
            # + 0.5 * A_class_weights * tf.keras.losses.KLDivergence()(A_fa, (B_img+A_fa)/2.)
            # realA_fakeA_loss = 0.5 * A_class_weights * tf.keras.losses.KLDivergence()(A_img, (A_img + B_fa)/2.) \
            # + 0.5 * B_class_weights * tf.keras.losses.KLDivergence()(B_fa, (A_img + B_fa)/2.)

            realB_fakeA_loss = 0.5 * tf.keras.losses.KLDivergence()(B_img, (A_fa+B_img)/2.) \
            + 0.5 * tf.keras.losses.KLDivergence()(A_fa, (B_img+A_fa)/2.)
            realA_fakeB_loss = 0.5 * tf.keras.losses.KLDivergence()(A_img, (A_img + B_fa)/2.) \
            + 0.5 * tf.keras.losses.KLDivergence()(B_fa, (A_img + B_fa)/2.)

            age_loss += (realB_fakeA_loss) + (realA_fakeB_loss)
